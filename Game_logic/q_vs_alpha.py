...
q_player = QLearning(epsilon=0.1, alpha=0.2, gamma=1, size=15, weights=[-18, -0.25, -0.25, -0.25, 18, -0.25, -0.25, -18, 0.625, 0.625])
...
def play_gomoku():
    ...
    while True: 
        while theWinner == 0:
            ...
            if current_player == PLAYER1 and PLTYP1 == 'q-learning':
                row, col = q_player.get_move(gomoku_board, current_player)
            elif current_player == PLAYER2 and PLTYP2=='alpha':
        ...
        if theWinner == PLAYER1:
            player_one_score += 1
            q_player.game_over(gomoku_board, current_player)
            break
...
def print_weights():
    ...
    filename = f"q_vs_alpha_{file_number}"
    ...
    with open(filename, 'a') as file:
        file.write(f"Q wins: {player_one_score}")
        file.write("  ")
        file.write(f"Alpha wins: {player_two_score}")
        file.write("\n")
