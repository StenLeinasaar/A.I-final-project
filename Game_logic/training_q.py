...
CHECKPOINT_PATH = default_checkpoint_path("q-learning", DEFAULT_ALPHA)
...
def save_weights(path=None, also_history=True):
    ...
    return save_agent_checkpoint(out, q_player_one, "q-learning", total_games, also_history=also_history)
...
def main(
    ...
    CHECKPOINT_PATH = out_path or default_checkpoint_path("q-learning", q_player_one.alpha)
    if resume_path:
        initial_weights_p1, initial_weights_p2 = apply_resume(
            resume_path, [q_player_one, q_player_two], stats
        )
    ...
    try:
        for i in range(total_games):
            ...
            display_progress(
                ...
                stats,
                [q_player_one, q_player_two],
                [initial_weights_p1, initial_weights_p2],
                ["Player 1 (Q-Learning)", "Player 2 (Q-Learning)"],
                CHECKPOINT_PATH,
                i,
                total_games,
                update_interval,
        ...
        if visualize and HAS_MATPLOTLIB:
            save_learning_curves(stats, "Q-Learning Training Progress", "q_learning_training_curves")
...
if __name__ == "__main__":
    args = make_arg_parser(
        "Self-play train Q-learning agents and save JSON weight checkpoints.",
        "weights/q_learning_alpha0.3.json",
    ).parse_args()
    main(
