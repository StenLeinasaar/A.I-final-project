...
def display_progress(current_game, total_games=None, update_interval=100, show_board=False):
    ...
    weight_change_p1 = sum(abs(sarsa_player_one.weights[i] - initial_weights_p1[i]) for i in range(len(initial_weights_p1)))
    weight_change_p2 = sum(abs(sarsa_player_two.weights[i] - initial_weights_p2[i]) for i in range(len(initial_weights_p2)))
    ...
    print(f"  Player 1 (SARSA): {win_rate_p1:.2f}% ({stats['player1_wins']} wins)")
    print(f"  Player 2 (SARSA): {win_rate_p2:.2f}% ({stats['player2_wins']} wins)")
    print("\n" + "-" * 80)
    ...
    print(f"\n  Player 1 Current Weights: {[f'{w:.3f}' for w in sarsa_player_one.weights]}")
    print(f"  Player 2 Current Weights: {[f'{w:.3f}' for w in sarsa_player_two.weights]}")
    print("\n" + "-" * 80)
    print("TRAINING PARAMETERS:")
    print(f"  Epsilon (exploration): {sarsa_player_one.epsilon}")
    print(f"  Alpha (learning rate): {sarsa_player_one.alpha}")
    print(f"  Gamma (discount): {sarsa_player_one.gamma}")
    print("=" * 80)
...
def save_learning_curves():
    ...
    # Recent win rates (last 1000 games)
    ...
    plt.tight_layout()
    filename = f"sarsa_training_curves_{int(time.time())}.png"
